...
include_once(_PS_MODULE_DIR_.'/tpaga/tpaga.php');

class TpagaController extends FrontController
{
	public $ssl = true;
	...
	public function process()
	{
		...
		self::$smarty->assign(array(
			'formLink' => Configuration::get('TPAGA_DEMO') != 'yes' ? 'https://gatewaylap.pagosonline.net/ppp-web-gateway/' :
	...
	public function displayContent()
	{
		...
		self::$smarty->display(_PS_MODULE_DIR_.'tpaga/views/templates/front/redirect.tpl');
	}
...
public function initParams()
{
	...
	$params = array(
		...
		'responseUrl'),
		array('value' => 'http://'.htmlspecialchars($_SERVER['HTTP_HOST'], ENT_COMPAT, 'UTF-8').__PS_BASE_URI__.'modules/tpaga/validation.php', 'name' => 'confirmationUrl'),
	);
	...
	{
		$tpaga = new Tpaga();
		$tpaga->validateOrder((int)self::$cart->id, (int)Configuration::get('PAYU_WAITING_PAYMENT'), (float)self::$cart->getOrderTotal(),
		$tpaga->displayName, null, array(), null, false,	self::$cart->secure_key);
	}
}

$tpagaController = new TpagaController();

if (Tools::getIsset(Tools::getValue('create-pending-order')))
	$tpagaController->createPendingOrder();
else
	$tpagaController->run();
